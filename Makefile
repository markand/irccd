...
LIBEV_CFLAGS += -DEV_IDLE_ENABLE=1
...
LIBNCE_DEPS = $(LIBNCE_SRCS:.c=.d)

LIBNCE_CFLAGS += $(LIBMINICORO_CFLAGS)
LIBNCE_CFLAGS += $(LIBEV_CFLAGS)
LIBNCE_CFLAGS += -I$(LIBNCE_DIR)
LIBNCE_CFLAGS += -DMCO_DEFAULT_STORAGE_SIZE=32768

LIBNCE_LDFLAGS += $(LIBMINICORO_LDFLAGS)
LIBNCE_LDFLAGS += $(LIBEV_LDFLAGS)

LIBNCE = $(LIBNCE_DIR)/libnce.a
LIBNCE_STATIC = $(LIBNCE) $(LIBMINICORO_STATIC) $(LIBEV_STATIC)

$(LIBNCE): $(LIBNCE_OBJS)
$(LIBNCE_OBJS): private override CFLAGS += $(LIBNCE_CFLAGS)

all:: $(LIBNCE)

clean::
	rm -f $(LIBNCE) $(LIBNCE_DEPS) $(LIBNCE_OBJS)
...
LIBIRCCD_SRCS += $(LIBIRCCD_DIR)/irccd/conn.c
...
LIBIRCCD_STATIC = $(LIBIRCCD) $(LIBNCE_STATIC)
